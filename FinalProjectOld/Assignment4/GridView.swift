...
//  Assignment4
//
//  Created by Micah Reid on 7/18/16.
...
class GridView: UIView {
    ...
    required init( grid: GridProtocol){
        self.rows = grid.rows
        self.cols = grid.cols
        self.grid = grid.grid
    ...
    override func drawRect(rect: CGRect) {
        ...
        let circleWidth: CGFloat = width / CGFloat(grid.maxDimension)
        let yIndent: CGFloat = CGFloat((grid.maxDimension - cols)/2)
        let xIndent: CGFloat = CGFloat((grid.maxDimension - rows)/2)
